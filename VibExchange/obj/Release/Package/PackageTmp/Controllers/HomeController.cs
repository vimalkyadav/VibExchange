...
namespace VibExchange.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.Message = "Welcome in EmailVerification. ";
            return View();
        }
        ...
        /// <summary>
        /// for  back end entry data can be  upload download button
        /// </summary>
        /// <returns></returns>
        public ActionResult getUploadButtonStatus()
        {
             //Check for balance
            //TODO: actual implementation
            using (DBClass context = new DBClass())
            {
                DataTable dtBalance = context.getData("Select Balance_point, ExpiryDate from tblUserPlans where UserID = (Select [AutoId] from [UserDetail] where [UserName]='" + Session["UserName"].ToString() + "')", CommandType.Text);
                if (dtBalance.Rows.Count > 0)
                {
                    int balancePoint = Convert.ToInt32(Convert.ToString(dtBalance.Rows[0]["Balance_point"]));
                    DateTime expirydate = Convert.ToDateTime(dtBalance.Rows[0]["ExpiryDate"]).Date;
                    if (balancePoint > 0)
                    {
                       // enable upload data button from profile
                        return Json(new { sts = "True", message = "True" });
                    }
                    else
                    {
                       // disable upload data button from profile
        ...
        public ActionResult GetPdf(string fileName)
        {
            ...
            return File(fs, "application/txt");
        }
        ...
        public ActionResult UserRegister(UserRegisterViewModel model, FormCollection form)
        {
            if (ModelState.IsValid)
            {
                ...
                Guid EmailVerification = Guid.NewGuid();
                try
                {
                    ...
                    using (DBClass context = new DBClass())
                    {
                        ...
                        context.AddParameter("@EmailVerification", EmailVerification.ToString());
        ...
        /// <summary>
        /// for Email verification of client
        /// </summary>
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult EmailVerification(UserRegisterViewModel model, FormCollection form)//public JsonResult RegisterConfirm(int regId)
        {
            var msg = "Your Email Is not Verified!";
            using (DBClass context = new DBClass())
            {
                DataTable dtRegister = context.getData("Select * from UserDetail where EmailVerification ='" + model.EmailVerification + "'", CommandType.Text);
                if (dtRegister.Rows.Count > 0)
                {
                    DataRow dr = dtRegister.Rows[0];
                    bool alreadyVerified = false;
                    try
                    {
                       alreadyVerified= Convert.ToBoolean(dr["IsVerified"].ToString());                      
                    }
                    catch
                    {}
                    if(alreadyVerified)
                    ...
                    else
                    {
                        ...
                        context.ExecuteNonQuery("Update UserDetail set IsVerified = 'True' where EmailVerification ='" + model.EmailVerification + "'", CommandType.Text);
                    }
                }
          ...
          public ActionResult ContactVerification()
          {
        ...
        //          string message = "Your OTP Number is " + otpValue + " ( 9871125850)";
